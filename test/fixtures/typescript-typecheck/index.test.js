...
test('shows error overlay in browser using babel mode', async () => {
  const { port, done } = await testSetup.scripts.start({
    env: { BABEL_ONLY: 'true' },
  });
...
test('shows error in console (dev mode) using babel mode', async () => {
  const { stderr } = await testSetup.scripts.start({
    smoke: true,
    env: { BABEL_ONLY: 'true' },
  });
  expect(stderr).toContain(expectedErrorMsg);
...
test('shows error in console (prod mode) using babel mode', async () => {
  const { stderr } = await testSetup.scripts.build({
    env: { BABEL_ONLY: 'true' },
  });
  expect(stderr).toContain(expectedErrorMsg);
